...
- **✅ G1 — Naming (DECIDED by Yu, 2026-07-07): keep TaxSorted.io.** Evidence: `regs/research/naming-clearance.md` (2026-07-03). Yu accepts the residual risk with eyes open — the MTDsorted® registered mark (cl 9 "tax preparation software") and the live taxsorted.co.uk CIS firm are known; fallback if ever challenged is rename-then (accepted, knowing post-listing rename is the expensive path) or defend slowly. Mitigations to execute now: register taxsorted.uk (free at Nominet as of 2026-07-03 — defensive grab, Yu's registrar action), non-association note in the site footer, always present as "TaxSorted.io", keep "MTD" out of brand-adjacent taglines, ™ only (never ®). Optional stress-test: UK TM application (~£170+£50/class) — not decided, still no paid brand spend committed.
- **✅ G2 — Liability shield (DECIDED by Yu, 2026-07-07): Mindicraft Limited, company no. 13782045.** Yu's existing UK private limited company (inc. 3 Dec 2021, St. John's Innovation Centre, Cambridge CB4 0WS; verified at Companies House 2026-07-07: active, filings current — next accounts due 30 Sep 2026, next confirmation statement due 9 Dec 2026; SIC 72190 + 77400). Deliberately NOT Cambridge TCG. Consequences to wire before the filing door opens: (a) Mindicraft becomes the Developer Hub organisation + data controller — the named "responsible individual" stays a natural person (Yu) acting as its director, so the entity absorbs the civil/regulatory exposure; (b) operating taxsorted likely ends dormant-accounts eligibility — notify HMRC active-for-CT within 3 months of first activity, file micro-entity accounts + nil CT return thereafter; (c) ICO registration (G4) in Mindicraft's name; (d) add a software SIC (62012) at the next confirmation statement; (e) PI/cyber insurance + terms/disclaimer architecture still required before filing opens. M1 prep tools ship under an education-only disclaimer meanwhile. Mitigating fact to document in terms: HMRC's Calculations API computes the tax.
...
- **M3 (Sept–Oct):** approval gauntlet — minimum-standards mapping doc, terms-of-use evidence pack (encryption ✓, GDPR ✓, WCAG audit ✓, pen test, support model), SDSTeam sandbox-log demonstration, production checklist, review rounds. ✅ G1 naming + ✅ G2 entity resolved 2026-07-07 (§9). **Target: listed before 7 Nov.**
...
2. ~~**G1 naming**~~ — **RESOLVED 2026-07-07: keep TaxSorted.io** (Yu; risk accepted, mitigations in §9).
3. ~~**G2 entity**~~ — **RESOLVED 2026-07-07: Mindicraft Limited, 13782045** (Yu; existing dormant ltd, deliberately not Cambridge TCG; consequences in §9).
