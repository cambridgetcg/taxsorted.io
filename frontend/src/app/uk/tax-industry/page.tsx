...
export const metadata: Metadata = {
  ...
  description:
    "A source-linked map of UK tax roles, qualifications, legal gates, professional rules, career paths, barriers, pay and institutional origins.",
};
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs font-semibold">
      {sources.map((source) => (
        <a
          ...
          className="text-accent underline decoration-line underline-offset-4 hover:text-accent-deep"
        >
          {source.title} ↗
        </a>
      ))}
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
      <header className="relative overflow-hidden rounded-[2rem] border border-line bg-ink p-6 text-white shadow-sm sm:p-10 lg:p-12">
        ...
        <div className="relative max-w-5xl">
          ...
          <p className="mt-5 max-w-4xl text-lg leading-8 text-white/75">
            Roles, exams, experience, licences, body rules, employer filters, study material,
            pay and ownership economics—shown as separate things, with the lawful
            lowest-friction route beside each barrier.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            <a
              href="https://api.taxsorted.io/v1/tax-industry/uk"
              target="_blank"
              rel="noreferrer noopener"
      ...
      <section className="mt-7" aria-labelledby="core-truth-title">
        <div className="overflow-hidden rounded-3xl border border-line bg-white shadow-sm">
          ...
          <p className="border-t border-line px-6 py-4 text-xs leading-5 text-ink-soft sm:px-8">
      ...
      <nav className="mt-6 rounded-3xl border border-line bg-white p-4" aria-label="On this page">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {[
            ["#rules", "Law, body or employer?"],
            ["#roles", "Roles"],
            ["#qualifications", "Qualifications & tests"],
            ["#routes", "Walk-through routes"],
            ["#gates", "All gates"],
            ["#barriers", "How barriers work"],
            ["#economics", "Pay & firm economics"],
            ["#origins", "Origins & power"],
          ].map(([href, label]) => (
            <a
              key={href}
              href={href}
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-3">
          {[
            ...
            <article key={gate.id} className="rounded-3xl border border-line bg-white p-6 shadow-sm" data-testid="rule-card">
              ...
              <p className="mt-3 text-sm leading-6 text-ink-soft">{truth}</p>
              <p className="mt-4 rounded-2xl bg-paper p-4 text-sm text-ink">
      ...
      <section id="roles" className="mt-16 scroll-mt-6" aria-labelledby="roles-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">What people actually do</p>
        <h2 id="roles-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">
          ...
          {industry.roles.map((role) => {
            ...
            return (
              <article key={role.id} className="flex flex-col rounded-3xl border border-line bg-white p-6 shadow-sm" data-testid="role-card">
                ...
                <dl className="mt-5 space-y-3 rounded-2xl bg-paper p-4 text-xs">
                  <div>
      ...
      <section id="qualifications" className="mt-16 scroll-mt-6" aria-labelledby="qualifications-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Tests · experience · recurring status</p>
        <h2 id="qualifications-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">
          Passing exams is only one part of qualifying.
        </h2>
        <p className="mt-3 max-w-4xl text-ink-soft">
          Open each route to see entry requirements, every assessment group, direct cost,
          experience sign-off, free or paid study material and what the credential does not authorise.
        </p>
        ...
        <div className="mt-7 space-y-4">
          {industry.qualifications.map((qualification) => {
            ...
            return (
              <details key={qualification.id} className="group rounded-3xl border border-line bg-white shadow-sm" data-testid="qualification-card">
                <summary className="cursor-pointer list-none p-5 marker:hidden sm:p-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-line text-lg text-accent group-open:rotate-45" aria-hidden="true">+</span>
                  </div>
                </summary>
                  ...
                  <div>
                    ...
                    <ol className="mt-5 grid gap-2 sm:grid-cols-2">
                      {qualification.assessments.map((assessment, index) => (
                        <li key={`${assessment.name}-${index}`} className="rounded-2xl bg-paper p-3 text-xs text-ink-soft">
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                      {resources.map((resource) => (
                        ...
                        >
                          ...
                          <strong className="mt-1 block text-sm text-ink">{resource.name} ↗</strong>
                          <span className="mt-2 block text-xs text-ink-soft">{resource.costAndAccess}</span>
                        </a>
                      ))}
                    </div>
      ...
      <section id="routes" className="mt-16 scroll-mt-6" aria-labelledby="routes-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">From starting point to stop condition</p>
        <h2 id="routes-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">
        ...
        <div className="mt-7 grid gap-5 xl:grid-cols-2">
          {industry.pathways.map((pathway) => (
            <article key={pathway.id} className="rounded-[2rem] border border-line bg-white p-6 shadow-sm sm:p-7" data-testid="pathway-card">
              ...
              <p className="mt-3 text-sm leading-6 text-ink-soft">{pathway.outcome}</p>
                  ...
                  <li key={step.order} className="grid grid-cols-[2rem_minmax(0,1fr)] gap-3">
                    <span className="flex h-8 w-8 items-center justify-center rounded-full bg-ink text-xs font-semibold text-white" aria-hidden="true">{step.order}</span>
                      ...
                      <p className="text-sm font-semibold text-ink">
                        {step.action}{" "}
                        <span className="text-[0.68rem] font-medium uppercase tracking-wide text-accent">
                          {step.mandatory ? "route step" : "optional or conditional"}
                        </span>
                      </p>
                      <p className="mt-1 text-xs leading-5 text-ink-soft">{step.why}</p>
                      <p className="mt-1 text-xs text-accent">{step.personalCost} · {step.time}</p>
                    </div>
                  </li>
                ))}
              </ol>
      ...
      <section id="gates" className="mt-16 scroll-mt-6" aria-labelledby="gates-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Trigger · controller · consequence</p>
        <h2 id="gates-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">
              ...
              <div key={status} className="rounded-[2rem] border border-line bg-white p-5 shadow-sm sm:p-7">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 max-w-3xl text-sm text-ink-soft">{copy.description}</p>
                  </div>
                  <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${copy.tone}`}>{gates.length} gates</span>
                </div>
                <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {gates.map((gate) => (
                    <details key={gate.id} className="group rounded-2xl border border-line p-4" data-testid="gate-card">
                      <summary className="cursor-pointer list-none marker:hidden">
                        <p className="text-[0.68rem] font-semibold uppercase tracking-wide text-accent">{gateAuthority(gate)}</p>
                        <h4 className="mt-1 font-semibold text-ink">{gate.name}</h4>
                        <p className="mt-2 text-xs text-ink-soft">{gate.trigger}</p>
                      </summary>
      ...
      <section id="barriers" className="mt-16 scroll-mt-6" aria-labelledby="barriers-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Barrier · rationale · escape route</p>
        ...
        <p className="mt-3 max-w-4xl text-ink-soft">
          The map names the mechanism, its stated justification, who carries the burden and a lawful route around needless friction.
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-2">
          {industry.barriers.map((barrier) => (
            <article key={barrier.id} className="rounded-3xl border border-line bg-white p-6 shadow-sm" data-testid="barrier-card">
              ...
              <p className="mt-3 text-sm leading-6 text-ink-soft">{barrier.mechanism}</p>
      ...
      <section id="economics" className="mt-16 scroll-mt-6" aria-labelledby="economics-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Salary ≠ revenue ≠ owner profit</p>
        <h2 id="economics-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">
        ...
        <p className="mt-3 max-w-4xl text-ink-soft">
          ...
          None of them proves what a qualification causes someone to earn.
        </p>
        <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {industry.compensation.map((item) => (
            <article key={item.id} className="rounded-3xl border border-line bg-white p-6 shadow-sm" data-testid="compensation-card">
              ...
              <p className="mt-2 text-xs text-ink-soft">per {humanise(item.period)} · {item.geography} · {item.referencePeriod}</p>
              <p className="mt-4 rounded-2xl bg-paper p-4 text-xs leading-5 text-ink-soft">
      ...
      <section id="origins" className="mt-16 scroll-mt-6" aria-labelledby="origins-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Origin · legal form · funding · limits</p>
        <h2 id="origins-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">
          The industry was built in layers, not designed in one room.
        </h2>
        <p className="mt-3 max-w-4xl text-ink-soft">
          State departments, chartered bodies, courts, commercial trainers, software platforms and partnerships
          hold different kinds of power. Their legal forms explain more than their logos do.
        </p>
        <div className="mt-7 grid gap-5 lg:grid-cols-2">
          {industry.institutions.map((institution) => (
            <article key={institution.id} className="rounded-3xl border border-line bg-white p-6 shadow-sm" data-testid="institution-card">
              ...
              <p className="mt-4 text-sm leading-6 text-ink-soft">{institution.origin}</p>
              ...
              <div className="mt-5 flex flex-wrap gap-4 text-xs font-semibold">
                <a href={institution.website} target="_blank" rel="noreferrer noopener" className="text-accent underline decoration-line underline-offset-4 hover:text-accent-deep">
                  Institution website ↗
                </a>
      ...
      <section className="mt-16 rounded-[2rem] border border-line bg-accent-soft p-6 sm:p-8" aria-labelledby="method-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">One reviewed corpus</p>
        <h2 id="method-title" className="mt-2 text-2xl font-semibold tracking-tight text-ink">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <a href="https://api.taxsorted.io/v1/tax-industry/uk/sources" target="_blank" rel="noreferrer noopener" className="rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white hover:bg-accent-deep">
            Browse the source ledger ↗
          </a>
          <a href="https://api.taxsorted.io/v1/tax-industry/uk/gaps" target="_blank" rel="noreferrer noopener" className="rounded-full border border-line bg-white px-4 py-2 text-sm font-semibold text-accent hover:bg-paper">
            See known gaps ↗
          </a>
          <a href="https://api.taxsorted.io/v1/tax-industry/uk/exports" target="_blank" rel="noreferrer noopener" className="rounded-full border border-line bg-white px-4 py-2 text-sm font-semibold text-accent hover:bg-paper">
            Download complete datasets ↗
          </a>
          <a href="https://api.taxsorted.io/v1/tax-industry/uk/dictionary" target="_blank" rel="noreferrer noopener" className="rounded-full border border-line bg-white px-4 py-2 text-sm font-semibold text-accent hover:bg-paper">
