...
function ApiLink({ path, children }: { path: string; children: React.ReactNode }) {
  return (
    <a
      ...
      className="break-all font-mono text-xs font-semibold text-accent underline decoration-line underline-offset-4 hover:text-accent-deep"
    >
      {children}
    </a>
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <header className="relative mt-8 overflow-hidden rounded-[2rem] border border-line bg-ink p-6 text-white shadow-sm sm:p-10">
        ...
        <div className="relative">
          ...
          <p className="mt-5 max-w-4xl text-lg text-white/75">
            Contracts, declared benefits, company records, land titles and political finance become
            separate, source-linked events. Law-enforcement power is mapped by institution and office.
            Nothing becomes a “deal”, favour or allegation merely because two names look alike.
          </p>
      ...
      <section className="mt-8" aria-labelledby="truth-rule">
        ...
        <div id="truth-rule" className="rounded-b-3xl border-x border-b border-line bg-accent-soft p-5 text-sm text-ink sm:px-6">
      ...
      <nav className="mt-6 rounded-3xl border border-line bg-white p-4" aria-label="On this page">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {[
            ...
            ["#enforcement", "Accountability map"],
            ["#power", "Office power cards"],
            ["#public-work", "People, work & language"],
            ["#api-doors", "API doors"],
            ["#boundaries", "Gates & exclusions"],
          ].map(([href, label]) => (
            <a key={href} href={href} className="rounded-full border border-line px-3 py-1.5 text-sm text-ink hover:border-accent hover:bg-accent-soft">
              {label}
            </a>
          ))}
        </div>
        ...
        <div className="mt-2 grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-4xl text-ink-soft">
              “Public registry” is the starting point, not automatic permission to copy everything.
              Copyright, database rights, personal-data necessity and source correction routes are checked separately.
            </p>
          </div>
          ...
          {financeDatasets.map((dataset) => (
            <article key={dataset.id} className="flex flex-col rounded-3xl border border-line bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[0.68rem] font-semibold uppercase tracking-wide ${statusTone[dataset.status]}`}>
                  {dataset.status}
                </span>
              </div>
              <p className="mt-3 text-sm text-ink-soft">{dataset.summary}</p>
              <div className="mt-5 space-y-3 text-xs">
                <p className="rounded-2xl bg-accent-soft p-3 text-ink"><strong>Safe projection:</strong> {dataset.published}</p>
              ...
              <div className="mt-auto flex flex-wrap gap-x-4 gap-y-2 pt-5 text-xs font-semibold">
                <a href={dataset.sourceUrl} target="_blank" rel="noreferrer noopener" className="text-accent underline decoration-line underline-offset-4">
                  Official source ↗
                </a>
      ...
      <section id="enforcement" className="mt-16 scroll-mt-6" aria-labelledby="enforcement-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Law enforcement · four constitutional maps</p>
        ...
        <p className="mt-3 max-w-4xl text-ink-soft">
          ...
          are different legal relationships. The map keeps them different, including across devolved systems.
        </p>
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-2">
          {accountabilityMaps.map((map) => (
            <article key={map.id} className="rounded-[2rem] border border-line bg-white p-6 shadow-sm sm:p-7">
              ...
              <p className="mt-5 rounded-2xl bg-accent-soft p-4 text-sm text-ink"><strong>Checks:</strong> {map.checks}</p>
            </article>
          ))}
        </div>

        <div className="mt-6 flex flex-wrap gap-3">
          <a href={`${politicsApiBase}/enforcement/institutions`} target="_blank" rel="noreferrer noopener" className="rounded-full bg-accent px-4 py-2 text-sm font-semibold text-white hover:bg-accent-deep">
            Browse institutions ↗
          </a>
          <a href={`${politicsApiBase}/enforcement/governance`} target="_blank" rel="noreferrer noopener" className="rounded-full border border-line bg-white px-4 py-2 text-sm font-semibold text-accent hover:bg-accent-soft">
            Inspect governance edges ↗
          </a>
          <a href={`${politicsApiBase}/enforcement/forces`} target="_blank" rel="noreferrer noopener" className="rounded-full border border-line bg-white px-4 py-2 text-sm font-semibold text-accent hover:bg-accent-soft">
            Open police-force directory ↗
          </a>
        </div>
      </section>

      <section id="power" className="mt-16 scroll-mt-6" aria-labelledby="power-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Seven visible dimensions · law-bound</p>
        <h2 id="power-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">
          Formal office power cards
        </h2>
        <p className="mt-3 max-w-4xl text-ink-soft">
          The number rates an office&apos;s published formal authority—not the holder&apos;s character,
            ...
            <article key={card.officeId} className="rounded-[2rem] border border-line bg-white p-6 shadow-sm sm:p-7">
              <header className="grid grid-cols-[minmax(0,1fr)_5.5rem] gap-4 border-b border-line pb-5">
                ...
                <div className="rounded-2xl bg-ink p-3 text-center text-white" aria-label={`Formal authority rating ${card.rating} out of 100`}>
                  ...
                  <p className="text-[0.65rem] uppercase tracking-wide text-white/65">of 100</p>
                </div>
              </header>
              ...
              <details className="mt-6 rounded-2xl bg-paper p-4">
                <summary className="cursor-pointer text-sm font-semibold text-ink">Read the office&apos;s legal limits</summary>
      ...
      <section id="public-work" className="mt-16 scroll-mt-6" aria-labelledby="public-work-title">
        <p className="text-sm font-semibold uppercase tracking-wide text-accent">Work, people and public words</p>
        ...
        <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          <article className="rounded-3xl border border-line bg-white p-6">
            ...
            <p className="mt-3 text-sm text-ink-soft">
              Pay bands, allowances, pensions and senior remuneration stay separate and effective-dated.
              Demographics are aggregate only, preserving official suppression and preventing small-cell differencing.
            </p>
          ...
          <article className="rounded-3xl border border-line bg-white p-6">
            ...
            <p className="mt-3 text-sm text-ink-soft">
              Vacancies link to official applications and never collect applicant data. Public work means
              board meetings, minutes, inspection reports, strategies, annual reports and official speeches—not officer logs.
            </p>
            <div className="mt-5 space-y-2">
          ...
          <article className="rounded-3xl border border-line bg-accent-soft p-6">
            <p className="text-xs font-semibold uppercase tracking-wide text-accent">Method only · review gate</p>
            <h3 className="mt-2 text-xl font-semibold text-ink">Observable language in official texts</h3>
            <p className="mt-3 text-sm text-ink-soft">
              A future reproducible corpus may count sentence length, questions, pronouns, modal words,
              numeric tokens and speaking-turn length. Minimum: 10 documents, 5,000 words, 3 dates and 30 days.
            </p>
          ...
          <article className="rounded-3xl border border-line bg-white p-6">
            ...
            <p className="mt-3 text-sm text-ink-soft">
              The SIA regulator, verified companies and public awards belong in the institution graph.
              The individual licence checker stays a purpose-bound official link, never a bulk worker directory.
            </p>
            <p className="mt-3 text-sm font-medium text-ink">
        ...
        <article className="mt-5 overflow-hidden rounded-3xl border border-line bg-white shadow-sm">
          ...
          <div className="grid gap-3 border-t border-line bg-accent-soft p-5 md:grid-cols-3">
            {englandWalesPolicePay.benefits.map((benefit) => (
              <p key={benefit} className="text-xs text-ink">{benefit}</p>
            ))}
          </div>
        </article>
      </section>
      ...
      <section id="api-doors" className="mt-16 scroll-mt-6" aria-labelledby="api-title">
        ...
        <h2 id="api-title" className="mt-2 text-3xl font-semibold tracking-tight text-ink">Developer handoff</h2>
        <p className="mt-3 max-w-3xl text-ink-soft">
          After this API release is deployed, the catalogue, rights statement and bulk dataset
          schemas remain readable through publication review. Record bodies and downloads remain
          closed until the human release conditions are met; the catalogue marks that state instead
          of implying an empty dataset. Once marked open, every dataset carries stable IDs, a schema,
          source IDs, licence notes and JSON, CSV and NDJSON links.
        </p>
        ...
        <p className="mt-5 text-sm font-semibold">
          <Link href="/uk/politics/api" className="text-accent underline underline-offset-4">
            Check release state and learn how to mirror with ETags →
          </Link>
        </p>
        <h3 className="mt-10 text-xl font-semibold text-ink">Purpose-built query and method doors</h3>
      ...
      <section id="boundaries" className="mt-16 scroll-mt-6" aria-labelledby="boundaries-title">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-2">
          <article className="rounded-3xl border border-amber-200 bg-amber-50 p-6">
            <h3 className="text-xl font-semibold text-amber-950">Gated pending a decision</h3>
            <ul className="mt-4 space-y-3 text-sm text-amber-950/80">
              {gatedIntegrityMaterial.map((item) => <li key={item}>○ {item}</li>)}
            </ul>
          </article>
          <article className="rounded-3xl border border-line bg-white p-6">
            <h3 className="text-xl font-semibold text-ink">Excluded by design</h3>
            <ul className="mt-4 space-y-3 text-sm text-ink-soft">
              {excludedIntegrityMaterial.map((item) => <li key={item}>— {item}</li>)}
            </ul>
          </article>
        </div>
        <p className="mt-6 rounded-2xl bg-ink p-5 text-sm text-white/80">
          <strong className="text-white">Emergency stop:</strong> every named-person publication gate can be
          closed without taking down the non-personal institution, method, source and accountability records.{" "}
          <ApiLink path="/integrity/corrections">Read the correction and restriction method ↗</ApiLink>
        </p>
      </section>

      <footer className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-line pt-6 text-sm text-ink-soft">
        <p>This is a public-accountability graph, not a universal people search.</p>
        <div className="flex gap-4 font-semibold">
          <Link href="/uk/politics/system" className="text-accent underline underline-offset-4">How political power works</Link>
          <Link href="/uk/politics/method" className="text-accent underline underline-offset-4">Publishing method</Link>
        </div>
      </footer>
    </div>
  );
