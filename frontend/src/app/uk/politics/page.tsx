...
const powerStyles: Record<string, string> = {
  ...
  "high-but-slow": "bg-paper text-ink",
};
          ...
          {politics.deciders.map((decider) => (
            <article key={decider.id} className="rounded-3xl border border-line bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`rounded-full px-3 py-1 text-xs font-medium ${powerStyles[decider.power] ?? "bg-paper text-ink"}`}>
                  {t("politics.power")}: {decider.power}
                </span>
              </div>
