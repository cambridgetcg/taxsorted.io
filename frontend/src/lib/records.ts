...
import { del as idbDel, get as idbGet, getMany as idbGetMany, set as idbSet, update as idbUpdate } from "idb-keyval";
import { validateLocalBooksState as validateState } from "@/lib/local-books-validation";
import {
  booksBackupDigest, exportBooksBackup, parseBooksBackup, restoredBooks, summariseBooksBackup,
  type BooksRestorePreflight,
} from "@/lib/books-backup";
import {
  BOOKS_STORAGE_KEY, BOOKS_LEGACY_STORAGE_KEY, BOOKS_MIGRATION_CLOSED_KEY, BOOKS_STORAGE_KEYS,
  BOOKS_CHANGED_MESSAGE, booksIdbStore, booksStorageFingerprint, replaceBooksInTransaction,
...
const STORAGE_KEY = BOOKS_STORAGE_KEY;
const STORAGE_KEY_V1 = BOOKS_LEGACY_STORAGE_KEY;
...
export interface RecordsStore {
  ...
  previewRestore(json: string): Promise<BooksRestorePreflight>;
  restoreBackup(json: string, preflight: BooksRestorePreflight): Promise<void>;
}
...
export interface RecordsBackend {
  ...
  /** Optional synchronous, all-or-nothing transaction for custom test backends. */
  transaction?(operation: (draft: RecordsBackend) => void): Promise<void>;
}
...
export function createRecordsStore(
  ...
  async function loadState(): Promise<LocalBooksState> {
    ...
    if (stored !== undefined) {
      ...
      const migrationClosed = await getValue<unknown>(BOOKS_MIGRATION_CLOSED_KEY);
      const legacy = migrationClosed === true ? undefined : await getValue<unknown>(STORAGE_KEY_V1);
  ...
  async function writeState(state: LocalBooksState): Promise<void> {
    validateState(state);
    if (useIdb) {
      // A tab which read the old installation before a restore must not write it
      // back afterwards, including browsers without Web Locks.
      await idbUpdate<unknown>(physicalKey(STORAGE_KEY), (current) => {
        if (isLocalBooksState(current) && current.replica.id !== state.replica.id) throw new Error(BOOKS_CHANGED_MESSAGE);
        return state;
      });
    } else await setValue(STORAGE_KEY, state);
  ...
  async function storageSnapshot(): Promise<unknown[]> {
    return useIdb
      ? idbGetMany(BOOKS_STORAGE_KEYS.map(physicalKey), booksIdbStore)
      : Promise.all(BOOKS_STORAGE_KEYS.map(key => getValue(key)));
  }

  function notifyRestored(): void {
    // Persistence has committed. A closed channel or failing view must never
    // turn that success into a reported restore failure.
    try { channel?.postMessage({ schema: LOCAL_BOOKS_SCHEMA, restored: true }); } catch { /* Refresh on next visit. */ }
    for (const listener of subscribers) {
      try { listener(); } catch { /* Other views still receive their notification. */ }
    }
  }

  function requireRestoreCoordination(): void {
    // Already-open /3 tabs predate the replica write guard. Their mutations use
    // this same Web Lock, so restoration is safe only when it is available.
    if (useIdb && (typeof navigator === "undefined" || typeof navigator.locks?.request !== "function")) {
      throw new Error("This browser cannot safely coordinate a Books restore. You can still download a backup. Restore it in a browser that supports Web Locks.");
    }
  }
  ...
  return {
    ...
    exportBackup: () => serialize(async () => exportBooksBackup(await loadState())),

    previewRestore: async (json) => {
      requireRestoreCoordination();
      const backup = await parseBooksBackup(json);
      const expectedBackupDigest = await booksBackupDigest(json);
      return serialize(async () => {
        const current = await loadState();
        const snapshot = await storageSnapshot();
        // A pre-v3 tab can still append to its own key during the preview. Such
        // records must be migrated into the safety copy before replacement.
        const legacy = snapshot[2] === true ? undefined : snapshot[1];
        if (canonicalAccountingJson(snapshot[0]) !== canonicalAccountingJson(current) ||
          (Array.isArray(legacy) && legacy.some(record => !current.events.some(event => event.id === record?.id)))) {
          throw new Error(BOOKS_CHANGED_MESSAGE);
        }
        return {
          expectedReplicaId: current.replica.id,
          expectedStoreRevision: current.storeRevision,
          expectedStorageFingerprint: booksStorageFingerprint(snapshot),
          expectedBackupDigest,
          existingBackup: await exportBooksBackup(current),
          existing: summariseBooksBackup(current), incoming: summariseBooksBackup(backup.books),
          exportedAt: backup.exportedAt,
        };
      });
    },

    restoreBackup: async (json, preflight) => {
      requireRestoreCoordination();
      const backup = await parseBooksBackup(json);
      if (await booksBackupDigest(json) !== preflight.expectedBackupDigest) throw new Error("The selected backup changed. Choose it again to review it.");
      const replacement = restoredBooks(backup, preflight.expectedStoreRevision);
      await serialize(async () => {
        // No loadState here: even legacy migrations must not write during a
        // failed restore. All compared keys and the marker commit together.
        if (useIdb) {
          await booksIdbStore("readwrite", store => replaceBooksInTransaction(
            store, BOOKS_STORAGE_KEYS.map(physicalKey), preflight.expectedStorageFingerprint, replacement,
          ));
        } else {
          const replace = (draft: RecordsBackend) => {
            const snapshot = BOOKS_STORAGE_KEYS.map(key => draft.get(key));
            if (booksStorageFingerprint(snapshot) !== preflight.expectedStorageFingerprint) throw new Error(BOOKS_CHANGED_MESSAGE);
            draft.set(STORAGE_KEY, structuredClone(replacement));
            draft.delete(STORAGE_KEY_V1);
            draft.set(BOOKS_MIGRATION_CLOSED_KEY, true);
          };
          if (map!.transaction) await map!.transaction(replace);
          else if (Object.getPrototypeOf(map) === Map.prototype) replace(map!);
          else throw new Error("This Books storage does not support a safe replacement.");
        }
      });
      notifyRestored();
    },
